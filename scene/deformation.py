...
class Deformation(nn.Module):
    def __init__(self, D=8, W=256, input_ch=27, input_ch_time=9, grid_pe=0,
                 skips=[], args=None, shs_pre=False, max_sh_degree=2):
        self.shs_pre, self.max_sh_degree = shs_pre, max_sh_degree
    ...
    def create_net(self):
        ...
        if self.shs_pre:
            self.rgb_deform = nn.Sequential(nn.ReLU(),
                                            nn.Linear(self.W, self.W),
                                            nn.ReLU(),
                                            nn.Linear(self.W, 3 * (self.max_sh_degree + 1) ** 2))
        else:
            self.rgb_deform = nn.Sequential(nn.ReLU(),
                                            nn.Linear(self.W, self.W),
                                            nn.ReLU(),
    ...
    def forward_dynamic(self, rays_pts_emb, scales_emb, rotations_emb, opacity_emb, shs_emb, time_feature, time_emb):
        ...
        if self.args.no_dshs:
            ...
            if self.shs_pre:
                dshs = self.rgb_deform(hidden).reshape([shs_emb.shape[0], 16, 3])
                shs = shs_emb * mask.unsqueeze(-1) + dshs
            else:
                dshs = self.rgb_deform(hidden).reshape([shs_emb.shape[0], 3])
                # shs = torch.zeros_like(shs_emb)
                shs = shs_emb * mask + dshs  # (shs_emb*mask).size()
...
class deform_network(nn.Module):
    def __init__(self, args, shs_pre=False, max_sh_degree=2):
        ...
        self.deformation_net = Deformation(W=net_width, D=defor_depth, input_ch=(3) + (3 * (posbase_pe)) * 2,
                                           ...
                                           shs_pre=shs_pre, max_sh_degree=max_sh_degree
                                           )
