...
def visual_result_mul(results, output_video_path='./exp_mul/1/', frame_rate=10):
    # 假设每个图像的大小为 [3, 128, 256]
    h, w = 128, 256
    ...
    for result in results:
        for index_image in range(view_num):
            # 提取并转置图像
            rgb_gt = np.transpose(result['image_all'][index_image].detach().cpu().numpy(), (1, 2, 0)) * 255
            # 转换 RGB 到 BGR
            rgb_gt = cv2.cvtColor(rgb_gt.astype(np.uint8), cv2.COLOR_RGB2BGR)

            # 生成文件名并保存
            frame_name = f'Frame_{index}_View_{index_image}.png'
            cv2.imwrite(os.path.join(output_video_path, frame_name), rgb_gt)
        index += 1
    ...
    for result in results:
        # 提取并转置图像
        rgb_gt = np.transpose(result['image_gts'][1].detach().cpu().numpy(), (1, 2, 0)) * 255
        # 转换 RGB 到 BGR
        rgb_gt = cv2.cvtColor(rgb_gt.astype(np.uint8), cv2.COLOR_RGB2BGR)

        # 生成文件名并保存
        frame_name = f'Frame_{index}_View_GTs.png'
        cv2.imwrite(os.path.join(output_video_path, frame_name), rgb_gt)
        index += 1
    ...
    for result in results:
        for index_image in range(view_num):
            # 提取并转置图像
            rgb_gt = np.transpose(result['image_shift'][index_image].detach().cpu().numpy(), (1, 2, 0)) * 255
            # 转换 RGB 到 BGR
            rgb_gt = cv2.cvtColor(rgb_gt.astype(np.uint8), cv2.COLOR_RGB2BGR)

            # 生成文件名并保存
            frame_name = f'Frame_{index}_View_{index_image}_Shift.png'
            cv2.imwrite(os.path.join(output_video_path, frame_name), rgb_gt)
        index += 1
...
def evaling(dataset, hyper, opt, pipe, checkpoint):
    ...
    for j, scenes in enumerate(train_dataloader):
        with accelerator.accumulate(gaussians):
            ...
            # 需要可视化，rgb，depth，seg，mesh， static rgb，dy rgb
            _step = int((time_length_load - time_length + 1) / time_length)
            gaussians.eval()
            result_diction = []
            for i in range(_step):
                viewpoint_stack_sub = viewpoint_stack[i * time_length * View_Num: (i + 1) * time_length * View_Num]
                result_diction_sub = gaussians.infer_mul(cams_input[:, i * time_length: (i + 1) * time_length, ...], cams[:,  i * time_length: (i + 1) *  time_length, ...],
                                                 depths[:,  i * time_length: (i + 1) *  time_length, ...],
                                                 semantic_mask[:,  i * time_length: (i + 1) *  time_length, ...],
                                                 intrinsics[:, i * time_length: (i + 1) *  time_length, ...], c2ws[:,  i * time_length: (i + 1) *  time_length, ...],
                                                 viewpoint_stack_sub,
                                                 pipe=pipe,
            ...
            visual_result_mul(result_diction, output_video_path="./exp_novel/" + "5000_" + str(j) + '/')
