...
class GaussianExtractor(object):
    def __init__(self, gaussians, bg_color=None):
        """
    ...
    @torch.no_grad()
    def reconstruction(self, result_diction, viewpoint_stack, length=3):
        """
        reconstruct radiance field given cameras
        """
        self.clean()
        self.viewpoint_stack = viewpoint_stack
        self.viewpoint_stack = self.viewpoint_stack[0:length]
        rgbs = result_diction['image_recstuctions']
        depths = result_diction['depth_recstuctions']
        self.gaussians = result_diction["Guassian_paras"][0]['means3D']
        for i in range(length):
            self.rgbmaps.append(rgbs[i].detach().cpu())
            self.depthmaps.append(depths[i].detach().cpu())
    ...
    def extract_mesh_unbounded(self, resolution=1024):
        ...
        R = contract(normalize(self.gaussians)).norm(dim=-1).cpu().numpy()
