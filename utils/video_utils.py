...
def render_func(
    ...
    with torch.no_grad():
        for i in tqdm(range(len(viewpoint_stack)), desc=f"rendering full data", dynamic_ncols=True):
            ...
            scence_index = int(i / 3)
            caminput = [viewpoint_stack[scence_index*3], viewpoint_stack[scence_index*3+1],
                                         viewpoint_stack[scence_index*3+2]]
            render_pkg = LRM_render(caminput, viewpoint_cam, gaussians, pipe, bg, return_decomposition=return_decomposition,return_dx=True)
        ...
        if len(dx_list)>1:
            ...
            for t in range(len(dx_list)): # 防止越界
                if t < len(dx_list)-num_cams:
                    ...
                    scence_index = int(t / 3)
                    caminput = [viewpoint_stack[scence_index * 3], viewpoint_stack[scence_index * 3 + 1],
                                viewpoint_stack[scence_index * 3 + 2]]
                    render_pkg2 = LRM_render(caminput, viewpoint_stack[t], gaussians, pipe, bg, override_color=ff_color)
                ...
                if t > num_cams-1:
                    ...
                    caminput = [viewpoint_stack[scence_index * 3], viewpoint_stack[scence_index * 3 + 1],
                                viewpoint_stack[scence_index * 3 + 2]]
                    render_pkg2 = LRM_render(caminput, viewpoint_stack[t], gaussians, pipe, bg, override_color=bf_color)
            ...
            for i, bf_color in enumerate(bf_color_first):
                ...
                caminput = [viewpoint_stack[scence_index * 3], viewpoint_stack[scence_index * 3 + 1],
                            viewpoint_stack[scence_index * 3 + 2]]
                render_pkg3 = LRM_render(caminput, viewpoint_stack[i], gaussians, pipe, bg, override_color=bf_color)
            ...
            for i, ff_color in enumerate(ff_color_last):
                ...
                render_pkg4 = LRM_render(caminput, viewpoint_stack[len(viewpoint_stack)-num_cams+i], gaussians, pipe, bg, override_color=ff_color)
