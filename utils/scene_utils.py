...
def render_training_image(scene, gaussians, viewpoints, render_func, pipe, background, stage, iteration, time_now):
    def render(cams_input, viewpoint, gaussians, path, scaling, background):
        # scaling_copy = gaussians._scaling
        render_pkg = render_func(cams_input, viewpoint, gaussians, pipe, bg_color=background, stage=stage)
        ...
        depth_np /= (depth_np.max()+0.000001)
    ...
    for idx in range(len(viewpoints)):
        scence_index = int(idx / 3)
        cam_input = [viewpoints[scence_index*3], viewpoints[scence_index*3+1],
                                         viewpoints[scence_index*3+2]]
        image_save_path = os.path.join(image_path, f"{iteration}_{idx}.jpg")
        render(cam_input, viewpoints[idx], gaussians, image_save_path, scaling = 1, background=background)
