...
class WaymoProcessor(object):
    ...
    def __init__(
        ...
        if "segmentation" in self.process_keys:
    ...
    def convert_one(self, file_idx):
        ...
        for frame_idx, data in enumerate(
            ...
            if "segmentation" in self.process_keys:
                self.save_segmentation(frame, file_idx, frame_idx)
            if frame_idx == 0:
    ...
    def _parse_function(self, proto):
        # 定义要解析的功能
        keys_to_features = {
            'context/scene/id': tf.io.FixedLenFeature([], tf.string),
            'context/frame_timestamp_micros': tf.io.FixedLenFeature([], tf.int64),
            'context/lidar_count': tf.io.FixedLenFeature([], tf.int64),
            'context/camera_count': tf.io.FixedLenFeature([], tf.int64),
            'context/camera_calibrations': tf.io.FixedLenFeature([], tf.string),
            'context/lidar_calibrations': tf.io.FixedLenFeature([], tf.string),
            'context/pose': tf.io.FixedLenFeature([], tf.string),
            'context/stats': tf.io.FixedLenFeature([], tf.string),
            'lidar/top/pose': tf.io.FixedLenFeature([], tf.string),
            'lidar/top/points': tf.io.VarLenFeature(tf.float32),
            'lidar/top/labels': tf.io.VarLenFeature(tf.float32),
            'images/front': tf.io.VarLenFeature(tf.string),
        }
        ...
        # 解析 LiDAR 数据
        lidar_points = tf.sparse.to_dense(parsed_features['lidar/top/points'])
        lidar_labels = tf.sparse.to_dense(parsed_features['lidar/top/labels'])

        return lidar_points, lidar_labels
    ...
    def save_lidar(self, frame, file_idx, frame_idx):
        ...
        origins, _ = convert_range_image_to_point_cloud(
            frame,
            range_images,
            camera_projections,
            range_image_top_pose
    ...
    def save_segmentation(self, frame, file_idx, frame_idx):
        """Parse and save the segmentation data.
        ...
        for img in frame.images:
            ...
            for label in frame.laser_labels:
                ...
                width = u.max() - u.min()
                height = v.max() - v.min()
                box_uvregions.append(np.array([u.min(), v.min(), u.max(), v.max()]))
                speeds.append(speed)

            box_uvregions = np.array(box_uvregions)
            speeds = np.array(speeds)
            mask = speeds > 0.01
            box_uvregions = box_uvregions[mask]
            self.predictor.set_image(image_input)
            input_boxes = torch.tensor(box_uvregions, device=self.predictor.device)
            transformed_boxes = self.predictor.transform.apply_boxes_torch(input_boxes, image_input.shape[:2])
            if transformed_boxes.shape[0]>0:
                masks, scores, _ = self.predictor.predict_torch(point_coords=None, point_labels=None, boxes=transformed_boxes, multimask_output=False)
                new_mask = torch.zeros(masks.shape[1:], dtype=torch.bool).to(masks[0].device)
                for mask in masks:
                    new_mask |= mask
                dynamic_mask = np.clip(new_mask.detach().cpu().numpy() * 255, 0, 255).astype(np.uint8)
                dynamic_mask = Image.fromarray(dynamic_mask.squeeze(0), "L")
                dynamic_mask_path = (
                    f"{self.save_dir}/{str(file_idx).zfill(3)}/sam_masks/"
                    + f"{str(frame_idx).zfill(3)}_{str(img.name - 1)}.png"
                )
                dynamic_mask.save(dynamic_mask_path)

            else:
                dynamic_mask = np.clip(np.ones(img_shape.shape[0:2])* 255, 0, 255).astype(np.uint8)
                dynamic_mask = Image.fromarray(dynamic_mask, "L")
                dynamic_mask_path = (
                        f"{self.save_dir}/{str(file_idx).zfill(3)}/sam_masks/"
                        + f"{str(frame_idx).zfill(3)}_{str(img.name - 1)}.png"
                ...
                dynamic_mask.save(dynamic_mask_path)
            #
            # masked_image = image_input * new_mask.permute(1, 2, 0).cpu().numpy()
            # masked_image_pil = Image.fromarray(masked_image.astype(np.uint8))
            # masked_image_pil.save('masked_image.png')
            # masked_image = image_input * (1-new_mask.permute(1, 2, 0).cpu().numpy())
            # masked_image_pil = Image.fromarray(masked_image.astype(np.uint8))
            # masked_image_pil.save('unmasked_image.png')

            # input_boxes = torch.tensor([[75, 275, 1725, 850],[425, 600, 700, 875]], device=self.predictor.device)
            # # thresholding, use 1.0 m/s to determine whether the pixel is moving
    ...
    def create_folder(self):
        ...
        for i in id_list:
            ...
            if "segmentation" in self.process_keys:
                os.makedirs(
                    f"{self.save_dir}/{str(i).zfill(3)}/sam_masks",
                    exist_ok=True,
                )
